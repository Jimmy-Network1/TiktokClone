...
const VideoItem: React.FC<VideoItemProps> = ({
  ...
  const likedByCurrentUser = useMemo(
    () =>
      !!session?.user &&
      likes.some((item: any) => item.user_id === session.user.id),
    [likes, session],
  );
  ...
  const bookmarkedByCurrentUser = useMemo(
    () =>
      !!session?.user &&
      bookmarks.some((item: any) => item.user_id === session.user.id),
    [bookmarks, session],
  );
  ...
  const handleLike = async (isDoubleTap = false, x = 0, y = 0) => {
    ...
    const nextLikes = isAddingLike
      ? [
          ...likes.filter((l: any) => l.user_id !== currentUserId),
  ...
  const handleBookmark = async () => {
    ...
    const nextBookmarks = isAddingBookmark
      ? [
          ...bookmarks.filter((b: any) => b.user_id !== currentUserId),
          { user_id: currentUserId },
        ]
    ...
    <View style={{ height, width }} className="relative bg-black">
      ...
      >
        <Animated.View style={[pauseIndicatorStyle, styles.overlayIcon]}>
          {isPaused ? (
            <Pause color="white" size={60} fill="white" />
          ) : (
            <Play color="white" size={60} fill="white" />
          )}
        </Animated.View>
      </View>
      ...
      <View className="absolute right-4 bottom-32 items-center space-y-7">
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('PublicProfile', { userId: video.userId })
          }
          className="mb-4"
        >
        ...
        >
          <Heart
            ...
            size={35}
            fill={
              likedByCurrentUser ? '#FE2C55' : 'rgba(255,255,255,0.9)'
            }
          />
        ...
        <TouchableOpacity className="items-center" onPress={handleBookmark}>
          <Bookmark
            ...
            size={35}
            fill={
              bookmarkedByCurrentUser ? '#FFB800' : 'rgba(255,255,255,0.9)'
            }
          />
      ...
      <View className="absolute bottom-6 left-4 right-20">
        ...
        <Text
          className="text-white text-sm mb-4 leading-5"
          numberOfLines={3}
        >
          {video.description}
        </Text>
